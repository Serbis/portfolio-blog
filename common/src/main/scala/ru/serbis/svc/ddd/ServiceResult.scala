...
object ServiceResult{
  ...
  def FsmResolvingFailure(fsm: String, ft: String) = Failure(FailureType.Internal, ErrorMessage(
    s"$fsm.PaymentCreationFsm", Some(s"Resolving timeout for $ft")))
...
object FailureType extends Enumeration{
  val Validation, Service, Internal, IncorrectCommand = Value
}
